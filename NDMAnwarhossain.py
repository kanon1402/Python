...
legend_element = [Patch(facecolor='#f9ff00', label='BOG'),
                  Patch(facecolor='#c085e9', label='FRD'),
                  Patch(facecolor='#ff8600', label='MYM'),
                  Patch(facecolor='#5d87a3', label='RAJ'),
                  Patch(facecolor='#c1f588', label='SAV'),
                  Patch(facecolor='#e985d6', label='TGL')
                  ]
...
plt.title('Last 15 days avg stock : Mr. Anwarhossain', fontsize=18, fontweight='bold', color='#3e0a75')
plt.legend(handles=legend_element, loc='lower right', fontsize=11, bbox_to_anchor=[0.0, 0.45])
ax.axis('equal')

plt.tight_layout()
plt.savefig('T3(Anwarhossain).png')
# plt.show()

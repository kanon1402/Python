...
legend_element = [Patch(facecolor='#f9ff00', label='DNJ'),
                  Patch(facecolor='#c085e9', label='GZP'),
                  Patch(facecolor='#ff8600', label='HZJ'),
                  Patch(facecolor='#5d87a3', label='KRN'),
                  Patch(facecolor='#c1f588', label='KSG'),
                  Patch(facecolor='#e985d6', label='MOT'),
                  Patch(facecolor='#99d3e9', label='RNG')
                  ]
...
plt.title('Last 15 days avg stock : Mr. Atik', fontsize=18, fontweight='bold', color='#3e0a75')
plt.legend(handles=legend_element, loc='best', fontsize=11)
ax.axis('equal')

plt.tight_layout()
plt.savefig('T3(Atik).png')
# plt.show()
